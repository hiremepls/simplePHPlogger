...
// If temperature contains multiple values, find the max
if (strpos($temp, "\n") !== false) {
    $tempValues = array_map('trim', explode("\n", $temp));
    $tempValues = array_filter($tempValues, 'is_numeric');
    $temp = !empty($tempValues) ? max($tempValues) : 'NA';
}
...
echo "Data logged: Machine=$machine, Max Temp=$temp, RAM=$ram, CPU=$cpu";
?>
